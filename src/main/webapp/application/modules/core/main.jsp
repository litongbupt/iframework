...
<body class="mainBody">
	<iframe src="<%=modulesPath%>core/header.jsp" frameborder="0"
		id="header"></iframe>
	<iframe
		src="<%=modulesPath %>core/leftmenu.jsp?role=${sessionScope.user.role}"
		frameborder="0" class="leftMenu"></iframe>
	<div class="frameToggle"></div>
	<div class="rightContent" id="rightContent">
		<iframe src="<%=modulesPath%>core/indexContent.jsp" frameborder="0"
			style="height:100%; width:100%;" id="rightFrame"></iframe>
	</div>
	<div id="courseViewDialog" title="浏览课件">
		<iframe id="iframe1" src="<%=modulesPath%>course/course_view.jsp"
	...
	<script type="text/javascript">
					...
					.dialog(
							{
								...
								close : function() {
									...
									if (url.substring(url.lastIndexOf("/")) != '/course_view.jsp') {
										window.document
												.getElementById("iframe1").src = "/DRTS/course/course_view.jsp";
									} else {
										$("#iframe1").contents().find(
												"#content_body").empty();
									}
								}
							});
		...
		function viewCourse(content) {
			$("#iframe1").contents().find("#content_body").removeClass(
					"welcome");
