$(function(){
	...
	$("#userList").jqGrid({
	    ...
	    colModel : [   
			{name:"userId",index:"user_id",label : "用户ID",width:30,align:"center", key:true},
...
//确认删除
function delUserCF(user_ID) {
	confirmJQ("确认删除此用户吗？", delUser, user_ID);
}

//删除试卷生成策略
function delUser(user_ID) {
	$('#activity_pane').showLoading();
	$.post(golbal_context+"/system/user/deleteUser.do", {user_ID : user_ID}, function(data) {
		$('#activity_pane').hideLoading();
		if (data == true) {
			alertJQ("操作成功，请等待页面刷新", 1000);
			$("#userList").jqGrid("delRowData", user_ID);
		} else if (data == false) {
			alertJQ("系统内部错误，请谅解！将尽快修复");
		}
	});
...
//重置用户密码入口
function recoverUserPassIn(user_ID) {
	confirmJQ("是否要恢复该用户的初始密码？", recoverUserPass, user_ID);
}

//重置用户密码策略
function recoverUserPass(user_ID) {
	$('#activity_pane').showLoading();

	$.post(golbal_context+"/system/user/recoverUserPass.do", {user_ID : user_ID}, function(data) {
		$('#activity_pane').hideLoading();
		if (data == true) {
...
function query() {
	...
	var postData = {
		user_ID : $("#query_user_ID").val().trim(),
